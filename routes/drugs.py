...
from shared_db import drugs_table as drugs, patients_table as patients
from utils.drug_values import parse_drug_form, drug_stock_details
import math
import json
import os
...
def manage_drugs():
    ...
    if request.method == 'POST':
        values, errors = parse_drug_form(request.form)
        if not errors:
            drugs.insert(values)
    ...
    return render_template('drugs.html', drugs=all_drugs, errors=errors,
                           form_values=request.form), 400 if errors else 200
...
def edit_drug(drug_id):
    ...
    errors = []
    if request.method == 'POST':
        values, errors = parse_drug_form(request.form, drug)
        if not errors:
            drugs.update(values, doc_ids=[drug_id])
            return redirect(url_for('drugs.manage_drugs'))

    from routes.mua_thuoc import purchases_table
    purchase_history, stock = drug_stock_details(
        drug.get('name'), purchases_table.all(), patients.all())
    return render_template('edit_drug.html', drug=drug,
                           form_values=request.form if errors else drug,
                           purchase_history=purchase_history, stock=stock,
